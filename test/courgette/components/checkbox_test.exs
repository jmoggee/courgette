defmodule Courgette.Components.CheckboxTest do
  use ExUnit.Case, async: false
  use Courgette.ComponentTestHelpers

  alias Courgette.Components.Checkbox
  alias Courgette.ComponentRegistry

  setup do
    ComponentRegistry.create_table()
    on_exit(fn -> ComponentRegistry.destroy_table() end)
  end

  defmodule Host do
    use Courgette.LiveComponent

    @impl true
    def mount(assigns) do
      {:ok,
       assigns
       |> assign_new(:checked, fn -> false end)
       |> assign_new(:label, fn -> nil end)
       |> assign_new(:on_change, fn -> nil end)
       |> assign_new(:last_change, fn -> nil end)}
    end

    @impl true
    def render(assigns) do
      live_component(Checkbox,
        id: "cb",
        checked: assigns.checked,
        label: assigns.label,
        on_change: assigns.on_change,
        focusable: true
      )
    end

    @impl true
    def handle_info({:toggled, value}, assigns) do
      {:noreply, assign(assigns, :last_change, value)}
    end

    def handle_info({:set, key, val}, assigns) do
      {:noreply, assign(assigns, key, val)}
    end

    def handle_info(_msg, assigns) do
      {:noreply, assigns}
    end
  end

  test "renders unchecked by default" do
    view = mount(Host)
    text = render_text(view)
    assert text =~ "[ ]"
  end

  test "renders checked when checked: true" do
    view = mount(Host, initial_assigns: %{checked: true})
    text = render_text(view)
    assert text =~ "[x]"
  end

  test "renders label text" do
    view = mount(Host, initial_assigns: %{label: "Accept terms"})
    text = render_text(view)
    assert text =~ "[ ]"
    assert text =~ "Accept terms"
  end

  test "space toggles from unchecked to checked" do
    view = mount(Host)
    text = render_text(view)
    assert text =~ "[ ]"

    send_tab(view)
    send_event(view, {:key, {:char, " "}})
    text = render_text(view)
    assert text =~ "[x]"
  end

  test "enter toggles from checked to unchecked" do
    view = mount(Host, initial_assigns: %{checked: true})
    text = render_text(view)
    assert text =~ "[x]"

    send_tab(view)
    send_event(view, {:key, :enter})
    text = render_text(view)
    assert text =~ "[ ]"
  end

  test "sends on_change message to parent with new boolean value" do
    view = mount(Host, initial_assigns: %{on_change: :toggled})
    send_tab(view)
    send_event(view, {:key, {:char, " "}})

    :sys.get_state(view.server)
    state = :sys.get_state(view.server)
    assert state.assigns.last_change == true
  end

  test "no notification without on_change prop" do
    view = mount(Host, initial_assigns: %{on_change: nil})
    send_tab(view)
    send_event(view, {:key, {:char, " "}})

    state = :sys.get_state(view.server)
    assert state.assigns.last_change == nil
  end

  test "focus/blur changes border color" do
    view = mount(Host)
    tree = render_tree(view)
    assert find_border_color(tree) == :white

    send_tab(view)
    tree = render_tree(view)
    assert find_border_color(tree) == :cyan
  end

  test "update checked via parent props" do
    view = mount(Host, initial_assigns: %{checked: false})
    text = render_text(view)
    assert text =~ "[ ]"

    send_info(view, {:set, :checked, true})
    text = render_text(view)
    assert text =~ "[x]"
  end

  # Helper to find border_color in tree
  defp find_border_color(nil), do: nil

  defp find_border_color(%Courgette.Element{type: :box, props: %{border_color: color}}), do: color

  defp find_border_color(%Courgette.Element{children: children}) do
    Enum.find_value(children, fn
      %Courgette.Element{} = child -> find_border_color(child)
      _ -> nil
    end)
  end
end
